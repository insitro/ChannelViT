...
class PatchEmbedPerChannel(nn.Module):
    ...
    def __init__(
        ...
        self.padding = nn.Parameter(torch.zeros((img_size, img_size)))
    ...
    def forward(self, x, extra_tokens={}):
        ...
        if self.training and self.enable_sample:
            # here we implement the simple channel dropout which is independent for each
            # channel
            sample = torch.bernoulli(torch.ones((B * Cin), device=x.device) * self.drop_rate)
            x = x.view(B*Cin, H, W)
            x[sample.nonzero()[:,0]] = self.padding
            x = x.view(B, Cin, H, W)
...
def samplev5channelvit_tiny(patch_size=16, **kwargs):
...
def samplev5channelvit_small(patch_size=16, **kwargs):
    model = ChannelVisionTransformer(
        patch_size=patch_size,
        embed_dim=384,
        depth=12,
...
def samplev5channelvit_base(patch_size=16, **kwargs):
    model = ChannelVisionTransformer(
        patch_size=patch_size,
        embed_dim=768,
        depth=12,
