    ...
    def forward(self, x, extra_tokens={}):
        ...
        for i, c in enumerate(cur_channels):
            ...
            hyper_weights, hyper_bias = self.hyper_network(
                self.channel_embed(torch.tensor(c).long())
            )
...
def hyperchannelvit_tiny(patch_size=16, **kwargs):
...
def hyperchannelvit_small(patch_size=16, **kwargs):
    model = ChannelVisionTransformer(
        patch_size=patch_size,
        embed_dim=384,
        depth=12,
...
def hyperchannelvit_base(patch_size=16, **kwargs):
    model = ChannelVisionTransformer(
        patch_size=patch_size,
        embed_dim=768,
        depth=12,
