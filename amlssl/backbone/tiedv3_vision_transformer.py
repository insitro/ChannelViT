...
class PatchEmbed(nn.Module):
    """Tied weights across channels. Image to Patch Embedding"""
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        self.proj = nn.Conv2d(
            1, embed_dim, kernel_size=patch_size, stride=patch_size, bias=True
        )
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            channels = random.sample(range(C), k=(C-C_new))
            x[:, channels, :, :] = 0

        # apply tied weights conv2d
        x = self.proj(x.view(-1, 1, H, W))  # B * C, Cout, Hout, Wout
        x = x.flatten(2).view(B, C, self.embed_dim, -1)  # B, C, Cout, Hout * Wout
        x = x.sum(dim=1).transpose(1, 2)  # B, Hout*Wout, C
...
def tiedv3vit_tiny(patch_size=16, **kwargs):
...
def tiedv3vit_small(patch_size=16, **kwargs):
    model = VisionTransformer(
        patch_size=patch_size,
        embed_dim=384,
        depth=12,
...
def tiedv3vit_base(patch_size=16, **kwargs):
    model = VisionTransformer(
        patch_size=patch_size,
        embed_dim=768,
        depth=12,
