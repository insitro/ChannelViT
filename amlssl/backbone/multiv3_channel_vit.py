...
from amlssl.backbone.vision_transformer import VisionTransformer
...
class MultiChannelVisionTransformer(nn.Module):
    ...
    def forward(self, x, extra_tokens={}):
        ...
        if self.training:
            ...
            # select the channels
            x = x[:,channels,:,:]
            cur_channels = cur_channels[channels]
...
def multiv3channelvit_tiny(patch_size=16, **kwargs):
...
def multiv3channelvit_small(patch_size=16, **kwargs):
    model = MultiChannelVisionTransformer(
        patch_size=patch_size,
        embed_dim=384,
        depth=12,
...
def multiv3channelvit_base(patch_size=16, **kwargs):
    model = MultiChannelVisionTransformer(
        patch_size=patch_size,
        embed_dim=768,
        depth=12,
