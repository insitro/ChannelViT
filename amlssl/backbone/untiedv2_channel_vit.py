...
class PatchEmbedPerChannel(nn.Module):
    ...
    def forward(self, x, extra_tokens={}):
        ...
        for i, c in enumerate(cur_channels):
            ...
            # get shuffled.
            out.append(self.projs[c](x[:,i,:,:].unsqueeze(1)))  # B Cout H W
        ...
        if self.training:
            ...
            channels = random.choices(range(Cin), k=Cin_new)
            Cin = Cin_new
            x = x[:,:,channels,:,:]
...
class ChannelVisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print("Warning!!!\n"
              "Untiedv2 channel vit randomly sample channels for each batch.\n"
...
def untiedv2channelvit_tiny(patch_size=16, **kwargs):
...
def untiedv2channelvit_small(patch_size=16, **kwargs):
    model = ChannelVisionTransformer(
        patch_size=patch_size,
        embed_dim=384,
        depth=12,
...
def untiedv2channelvit_base(patch_size=16, **kwargs):
    model = ChannelVisionTransformer(
        patch_size=patch_size,
        embed_dim=768,
        depth=12,
