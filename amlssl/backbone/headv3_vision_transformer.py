...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            selected = [c for c in range(C) if c not in channels]
        else:
            selected = list(range(C))

        channels = "".join([str(c) for c in sorted(selected)])
...
class VisionTransformer(nn.Module):
    ...
    def prepare_tokens(self, x):
        B, nc, w, h = x.shape
        x = self.input_drop(x)
        x, channels = self.patch_embed(x)  # patch linear embedding
    ...
    def forward(self, x, extra_tokens={}):
        x, channels = self.prepare_tokens(x)
...
def headv3vit_tiny(patch_size=16, **kwargs):
...
def headv3vit_small(patch_size=16, **kwargs):
    model = VisionTransformer(
        patch_size=patch_size,
        embed_dim=384,
        depth=12,
...
def headv3vit_base(patch_size=16, **kwargs):
    model = VisionTransformer(
        patch_size=patch_size,
        embed_dim=768,
        depth=12,
