...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            Cin = len(x.sum(dim=(0, 2, 3)).nonzero())
            if Cin == 8:
                C_new = random.randint(6, C)
            else:
...
def v4vit_tiny(patch_size=16, **kwargs):
...
def v4vit_small(patch_size=16, **kwargs):
    model = VisionTransformer(
        patch_size=patch_size,
        embed_dim=384,
        depth=12,
...
def v4vit_base(patch_size=16, **kwargs):
    model = VisionTransformer(
        patch_size=patch_size,
        embed_dim=768,
        depth=12,
