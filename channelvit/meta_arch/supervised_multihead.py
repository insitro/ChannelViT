...
class SupervisedMultihead(pl.LightningModule):
    def __init__(self, main_supervised_cfg: DictConfig) -> None:
        ...
        if self.cfg.multihead == "expert":
            self.channels_to_id = self.get_channels_to_id(self.cfg.backbone.args.in_chans)
            self.multi_head_classifier = nn.ModuleList([
                nn.Linear(self.backbone.embed_dim, self.cfg.num_classes)
                for _ in range(len(self.channels_to_id))
            ])
        elif self.cfg.multihead == "concat":
            self.channels_to_onehot = self.get_channels_to_onehot(self.cfg.backbone.args.in_chans)
            self.multi_head_classifier = nn.Sequential(
                nn.Linear(self.backbone.embed_dim + self.cfg.backbone.args.in_chans,
                          self.backbone.embed_dim * 2),
                nn.ReLU(),
                nn.Linear(self.backbone.embed_dim * 2, self.cfg.num_classes),
            )
        elif self.cfg.multihead == "concat-embed":
            self.channels_to_embed_id = self.get_channels_to_embed_id(self.cfg.backbone.args.in_chans)
            self.channel_embed = nn.Embedding(self.cfg.backbone.args.in_chans,
                                              self.backbone.embed_dim)
            self.multi_head_classifier = nn.Sequential(
                nn.Linear(self.backbone.embed_dim * 2,
                          self.backbone.embed_dim * 2),
                nn.ReLU(),
                nn.Linear(self.backbone.embed_dim * 2, self.cfg.num_classes),
            )
    ...
    def get_channels_to_id(self, max_channels: int) -> dict[str, int]:
        channels = list(range(max_channels))
        keys = []
        for n in range(len(channels), 0, -1):
            for selected in combinations(channels, n):
                selected = "".join([str(s) for s in list(selected)])
                keys.append(selected)
        return dict(zip(keys, range(len(keys))))

    def get_channels_to_onehot(self, max_channels: int):
        channels = list(range(max_channels))
        keys = []
        values = []
        for n in range(len(channels), 0, -1):
            for selected in combinations(channels, n):
                ...
                selected = "".join([str(s) for s in list(selected)])
                keys.append(selected)
                values.append(torch.tensor(cur_value, device=self.device).long())

        return dict(zip(keys, values))


    def get_channels_to_embed_id(self, max_channels: int):
        channels = list(range(max_channels))
        keys = []
        values = []
        for n in range(len(channels), 0, -1):
    ...
    def train_single_batch(self, batch):
        ...
        output, channels = self.backbone(imgs, covariates)

        if self.cfg.multihead == "expert":
            logit = self.multi_head_classifier[self.channels_to_id[channels]](output)
        elif self.cfg.multihead == "concat":
            one_hot = self.channels_to_onehot[channels].view(1,
                                                             -1).to(self.device).expand(len(output),
                                                                                        -1)
            logit = self.multi_head_classifier(torch.cat([output, one_hot], dim=-1))
        elif self.cfg.multihead == "concat-embed":
            selected_channels = self.channels_to_embed_id[channels].to(self.device)  # selected channels
            channel_embed = self.channel_embed(selected_channels)
            # sum over all selected channel embeddings and reshape it for broadcast
            channel_embed = channel_embed.sum(dim=0).unsqueeze(0).expand(len(output),
                                                                         -1)
            logit = self.multi_head_classifier(torch.cat([output, channel_embed], dim=-1))
    ...
    def inference(self, batch, batch_idx):
        ...
        if self.cfg.multihead == "expert":
            pred = self.multi_head_classifier[self.channels_to_id[channels]](output)
        elif self.cfg.multihead == "concat":
            one_hot = self.channels_to_onehot[channels].view(1,
                                                             -1).to(self.device).expand(len(output),
                                                                                        -1)
            pred = self.multi_head_classifier(torch.cat([output, one_hot], dim=-1))
        elif self.cfg.multihead == "concat-embed":
            selected_channels = self.channels_to_embed_id[channels].to(self.device)  # selected channels
            channel_embed = self.channel_embed(selected_channels)
            # sum over all selected channel embeddings and reshape it for broadcast
            channel_embed = channel_embed.sum(dim=0).unsqueeze(0).expand(len(output),
                                                                         -1)
            pred = self.multi_head_classifier(torch.cat([output, channel_embed], dim=-1))
    ...
    def configure_optimizers(self):
        ...
        params_groups[0]['params'] += self.multi_head_classifier.parameters()
        if self.cfg.multihead == "concat-embed":
            params_groups[0]['params'] += self.channel_embed.parameters()
